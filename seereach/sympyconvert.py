...
from seereach.symlang import STuple, SVariable, SInteger, SReal, SBoolean, SBinaryOp, SUnaryOp, Operator, Name, SymLang
from sympy import symbols, simplify, sin, cos, tan, sqrt, pi, E, log, Eq, Lt, Le, Gt, Ge, And, Or, Not
...
class SymPyConverter:
    def from_sympy(self, expr: sympy.Expr) -> SymLang:
        ...
        if isinstance(expr, Symbol):
            return SVariable(Name(str(expr)), Type.REAL if expr.is_real else Type.INTEGER)
            ...
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.ADD, self.from_sympy(expr.args[1]))
        elif isinstance(expr, sympy.Mul):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.MUL, self.from_sympy(expr.args[1]))
        elif isinstance(expr, sympy.Pow):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.POW, self.from_sympy(expr.args[1]))
        elif isinstance(expr, Eq):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.EQUAL, self.from_sympy(expr.args[1]))
        elif isinstance(expr, Lt):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.LESS, self.from_sympy(expr.args[1]))
        elif isinstance(expr, Le):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.LESS_EQUAL, self.from_sympy(expr.args[1]))
        elif isinstance(expr, Gt):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.GREATER, self.from_sympy(expr.args[1]))
        elif isinstance(expr, Ge):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.GREATER_EQUAL, self.from_sympy(expr.args[1]))
        elif isinstance(expr, And):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.AND, self.from_sympy(expr.args[1]))
        elif isinstance(expr, Or):
            return SBinaryOp(self.from_sympy(expr.args[0]), Operator.OR, self.from_sympy(expr.args[1]))
    ...
    def to_sympy(self, expr: SymLang):
        ...
        if isinstance(expr, SVariable):
            ...
            return sympy.logic.boolalg.BooleanTrue() if expr.value else sympy.logic.boolalg.BooleanFalse()
